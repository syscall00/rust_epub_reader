use druid::{
    widget::{Flex, Container, Controller, Label, Spinner}, Widget, WidgetExt, Env, EventCtx, Event, Insets,
};
...
pub fn build_ocr_ui() -> impl Widget<OcrData> {
    ...
    .with_tab_index(0).background(style::get_color_unchecked(style::PRIMARY_DARK));
...
fn find_by_photo() -> impl Widget<OcrData> {
    ...
    let result = Flex::row()
    .with_child(druid::widget::Label::new(|data: &OcrData, _env: &druid::Env| { data.ocr_result.to_string() }))
    .with_child(
    ...
    let either = druid::widget::Either::new(
        ...
        .with_child(Spinner::new()).center(),
        result,
    );
    
    let either_result = druid::widget::Either::new(
        |data: &OcrData, _env: &druid::Env| data.ocr_result != PagePosition::new(0, 0) || data.processing,
        either,
...
fn find_by_virtual() -> impl Widget<OcrData> {
    ...
    .on_click(|ctx, data: &mut OcrData, _| {
        ...
        ctx.submit_command(
            INTERNAL_COMMAND
                .with(
                    InternalUICommand::RequestReverseOCR((data.image_for_pos_1.clone(), data.image_for_pos_2.clone()))
                )
                .to(druid::Target::Global),
        );
        data.processing = true;
    ...
    let result = Flex::row()
    .with_child(druid::widget::Label::new(|data: &OcrData, _env: &druid::Env| { data.reverse_ocr_result.to_string() }))
    .expand_width();
    ...
    let either_result = druid::widget::Either::new(
        |data: &OcrData, _env: &druid::Env| data.reverse_ocr_result != PagePosition::new(0, 0) || data.processing,
        either,
        druid::widget::Label::new(""),
    );
    Flex::column()
...
fn explanation_text(mode: &OcrMode) -> impl Widget<OcrData> {
    ...
    }.with_text_size(22.).expand_width().padding(druid::Insets::new(5., 5., 5., 15.))
}